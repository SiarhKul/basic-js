...
class VigenereCipheringMachine {
  ...
  constructor(directOrRevese) {
    directOrRevese === undefined
      ? this.modeDirectRevese = true
      : this.modeDirectRevese = false
  }
  ...
  encrypt(str, key) {
    ...
    const keyArr = strArr.reduce((newArr, later, i) => {
      Boolean(later)
        ? newArr.push(key[i % key.length].toLowerCase())
        : null
      return newArr
    }, [])

    strArr.map((later, i) => {
      return Boolean(later.trim()) === Boolean((keyArr[i]).trim())
        ? keyArr[i]
        : keyArr.splice(i, 0, later)
    })

    const strArrNum = strArr.map(later => {
      return ALPHABET.includes(later)
        ? ALPHABET.indexOf(later)
        : later
    })
    ...
    const res = this.modeDirectRevese
      ? encryptNumToLater.join('').toUpperCase()
      : encryptNumToLater.reverse().join('').toUpperCase()
  ...
  decrypt(str, key) {
    ...
    const keyArr = strCtypted.reduce((newArr, later, i) => {
      Boolean(later)
        ? newArr.push(key[i % key.length].toUpperCase())
        : null
      return newArr
    }, [])
    ...
    const strCtyptedToNumber = strCtypted.map((later) => {
      return ALPHABET.includes(later.toLowerCase())
        ? ALPHABET.indexOf(later.toLowerCase())
        : later
    })
